...
echo "=== Filling gap for all exchanges (like Paradex) ==="
...
# Process each exchange separately with correct funding rate calculation
EXCHANGES=(
...
for config in "${EXCHANGES[@]}"; do
  ...
  # Aggregate with simplified symbol normalization
...
WHERE exchange = '$exchange' 
  AND created_at < $FIVE_MIN_AGO
" 2>&1 | grep -v "wrangler\|Resource\|Executing\|database\|command" | head -1
...
echo ""
echo "=== Testing API ==="
curl -s "https://defiapi.cloudflareone-demo-account.workers.dev/api/normalized-data?symbol=BTC&limit=20" | jq '[.data[] | {exchange, timestamp_iso, data_source}] | group_by(.exchange) | map({exchange: .[0].exchange, latest: .[0].timestamp_iso})'
