...
/**
 * HyperliquidTracker - Durable Object für API-basierte Datenabfrage
 *
 ...
 */
export class HyperliquidTracker implements DurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path !== '/stop' && !this.isRunning) {
      console.log('[HyperliquidTracker] Auto-starting tracker');
      await this.start().catch((error) => {
        console.error('[HyperliquidTracker] Auto-start failed:', error);
      });
    }
  ...
  private async start(): Promise<void> {
    ...
    this.isRunning = true;
    console.log('[HyperliquidTracker] Starting polling mechanism');
  ...
  private scheduleNextPoll(): void {
    ...
    console.log(`[HyperliquidTracker] Next poll scheduled in ${msUntilNext}ms (at :${nextTargetSeconds.toString().padStart(2, '0')})`);
  ...
  private stop(): void {
    ...
    console.log('[HyperliquidTracker] Polling stopped');
  }
   ...
   *
   * Ablauf:
   * 1. API aufrufen (metaAndAssetCtxs)
  ...
  private async pollAndSave(): Promise<void> {
    ...
    console.log(`[HyperliquidTracker] Poll #${this.pollCount} starting...`);
    ...
    try {
      ...
      if (universe.length !== assetCtxs.length) {
        console.warn(`[HyperliquidTracker] Array length mismatch: universe=${universe.length}, assetCtxs=${assetCtxs.length}`);
      }
      ...
      for (let i = 0; i < maxLength; i++) {
        ...
        if (universeItem && assetCtx && universeItem.name) {
          this.dataBuffer.set(universeItem.name, { universe: universeItem, assetCtx });
        }
      }

      console.log(`[HyperliquidTracker] Poll #${this.pollCount} fetched ${this.dataBuffer.size} assets`);
      ...
      console.error(`[HyperliquidTracker] Poll #${this.pollCount} failed:`, errorMessage);
  ...
  private async saveToDatabase(): Promise<void> {
    if (this.dataBuffer.size === 0) {
      console.log('[HyperliquidTracker] No data to save');
      return;
    }
    ...
    try {
      ...
      console.log('[HyperliquidTracker] Starting to process buffer for database save');
      ...
      for (const [symbol, data] of this.dataBuffer.entries()) {
        ...
        records.push({
          ...
          open_interest_limit: '0', // Hyperliquid hat dieses Feld nicht
          funding_clamp_small: '0', // Hyperliquid hat dieses Feld nicht
          funding_clamp_big: '0', // Hyperliquid hat dieses Feld nicht
          ...
          daily_price_low: 0, // Hyperliquid hat dieses Feld nicht direkt
          daily_price_high: 0, // Hyperliquid hat dieses Feld nicht direkt
      ...
      if (records.length === 0) {
        console.log('[HyperliquidTracker] No valid records to save');
        return;
      }
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error('[HyperliquidTracker] Failed to save to database:', errorMessage);
      console.error('[HyperliquidTracker] Records count:', records.length);

      if (records.length > 0) {
        console.error('[HyperliquidTracker] Sample record:', JSON.stringify(records[0]));
      }
  ...
  private startStatusCheck(): void {
    ...
    this.statusCheckInterval = setInterval(() => {
      console.log(`[HyperliquidTracker] Status Check - Running: ${this.isRunning}, Buffer: ${this.dataBuffer.size}, Polls: ${this.pollCount}`);
      console.log(`[HyperliquidTracker] Last poll: ${new Date(this.lastPollTime).toISOString()}`);
    }, 30000) as any;
  }
  ...
  private async updateTrackerStatus(status: string, errorMessage: string | null): Promise<void> {
    try {
      ...
      `).bind(
        ...
        now,
        errorMessage,
        0, // Hyperliquid hat keine Reconnects, immer 0
        now,
        'hyperliquid'
      ).run();
    } catch (error) {
      console.error('[HyperliquidTracker] Failed to update tracker status:', error);
    }
  }
}
