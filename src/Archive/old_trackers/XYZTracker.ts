...
/**
 * XYZTracker - Durable Object für API-basierte Datenabfrage (Trade.xyz)
 *
 ...
 */
export class XYZTracker implements DurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path !== '/stop' && !this.isRunning) {
      console.log('[XYZTracker] Auto-starting tracker');
      await this.start().catch((error) => {
        console.error('[XYZTracker] Auto-start failed:', error);
      });
    }
  ...
  private async start(): Promise<void> {
    ...
    this.isRunning = true;
    console.log('[XYZTracker] Starting polling mechanism');
  ...
  private scheduleNextPoll(): void {
    ...
    console.log(`[XYZTracker] Next poll scheduled in ${msUntilNext}ms (at :${nextTargetSeconds.toString().padStart(2, '0')})`);
  ...
  private stop(): void {
    ...
    console.log('[XYZTracker] Polling stopped');
  }
  ...
  private async pollAndSave(): Promise<void> {
    ...
    console.log(`[XYZTracker] Poll #${this.pollCount} starting...`);
    ...
    try {
      ...
      if (universe.length !== assetCtxs.length) {
        console.warn(`[XYZTracker] Array length mismatch: universe=${universe.length}, assetCtxs=${assetCtxs.length}`);
      }
      ...
      for (let i = 0; i < maxLength; i++) {
        ...
        if (universeItem && assetCtx && universeItem.name) {
          // Symbol direkt verwenden (API gibt bereits xyz:SYMBOL zurück)
          const symbol = universeItem.name;
          this.dataBuffer.set(symbol, { universe: universeItem, assetCtx });
        }
      }

      console.log(`[XYZTracker] Poll #${this.pollCount} fetched ${this.dataBuffer.size} assets`);
      ...
      console.error(`[XYZTracker] Poll #${this.pollCount} failed:`, errorMessage);
  ...
  private async saveToDatabase(): Promise<void> {
    if (this.dataBuffer.size === 0) {
      console.log('[XYZTracker] No data to save');
      return;
    }
    ...
    try {
      ...
      console.log('[XYZTracker] Starting to process buffer for database save');
      ...
      if (records.length === 0) {
        console.log('[XYZTracker] No valid records to save');
        return;
      }
      ...
      console.log(`[XYZTracker] ✅ Saved ${records.length} records to database`);
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error('[XYZTracker] ❌ Failed to save to database:', errorMessage);
      console.error('[XYZTracker] Records count:', records.length);

      if (records.length > 0) {
        console.error('[XYZTracker] Sample record:', JSON.stringify(records[0]));
      }
  ...
  private startStatusCheck(): void {
    ...
    this.statusCheckInterval = setInterval(() => {
      console.log(`[XYZTracker] Status Check - Running: ${this.isRunning}, Buffer: ${this.dataBuffer.size}, Polls: ${this.pollCount}`);
      console.log(`[XYZTracker] Last poll: ${new Date(this.lastPollTime).toISOString()}`);
    }, 30000) as any;
  }
  ...
  private async updateTrackerStatus(status: string, errorMessage: string | null): Promise<void> {
    try {
      ...
      console.error('[XYZTracker] Failed to update tracker status:', error);
    }
  }
}
