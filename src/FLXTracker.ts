...
/**
 * FLXTracker - Durable Object für API-basierte Datenabfrage
 *
 ...
 */
export class FLXTracker implements DurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path !== '/stop' && !this.isRunning) {
      console.log('[FLXTracker] Auto-starting tracker');
      await this.start().catch((error) => {
        console.error('[FLXTracker] Auto-start failed:', error);
      });
    }
  ...
  private async start(): Promise<void> {
    ...
    this.isRunning = true;
    console.log('[FLXTracker] Starting polling mechanism');
  ...
  private scheduleNextPoll(): void {
    ...
    console.log(`[FLXTracker] Next poll scheduled in ${msUntilNext}ms (at :${nextTargetSeconds.toString().padStart(2, '0')})`);
  ...
  private stop(): void {
    ...
    console.log('[FLXTracker] Polling stopped');
  }
  ...
  private async pollAndSave(): Promise<void> {
    ...
    console.log(`[FLXTracker] Poll #${this.pollCount} starting...`);

    try {
      // API aufrufen - Felix verwendet dex: "hyna" Parameter
      ...
      if (universe.length !== assetCtxs.length) {
        console.warn(`[FLXTracker] Array length mismatch: universe=${universe.length}, assetCtxs=${assetCtxs.length}`);
      }
      ...
      for (let i = 0; i < maxLength; i++) {
        ...
        if (universeItem && assetCtx && universeItem.name) {
          // Symbol direkt verwenden (API gibt bereits flx:SYMBOL zurück)
          const symbol = universeItem.name;
          this.dataBuffer.set(symbol, { universe: universeItem, assetCtx });
        }
      }

      console.log(`[FLXTracker] Poll #${this.pollCount} fetched ${this.dataBuffer.size} assets`);
      ...
      console.error(`[FLXTracker] Poll #${this.pollCount} failed:`, errorMessage);
  ...
  private async saveToDatabase(): Promise<void> {
    if (this.dataBuffer.size === 0) {
      console.log('[FLXTracker] No data to save');
      return;
    }
    ...
    try {
      ...
      console.log('[FLXTracker] Starting to process buffer for database save');
      ...
      for (const [symbol, data] of this.dataBuffer.entries()) {
        ...
        records.push({
          ...
          open_interest_limit: '0', // Felix hat dieses Feld nicht
          funding_clamp_small: '0', // Felix hat dieses Feld nicht
          funding_clamp_big: '0', // Felix hat dieses Feld nicht
          ...
          daily_price_low: 0, // Felix hat dieses Feld nicht direkt
          daily_price_high: 0, // Felix hat dieses Feld nicht direkt
      ...
      if (records.length === 0) {
        console.log('[FLXTracker] No valid records to save');
        return;
      }
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error('[FLXTracker] ❌ Failed to save to database:', errorMessage);
      console.error('[FLXTracker] Records count:', records.length);

      if (records.length > 0) {
        console.error('[FLXTracker] Sample record:', JSON.stringify(records[0]));
      }
  ...
  private startStatusCheck(): void {
    ...
    this.statusCheckInterval = setInterval(() => {
      console.log(`[FLXTracker] Status Check - Running: ${this.isRunning}, Buffer: ${this.dataBuffer.size}, Polls: ${this.pollCount}`);
      console.log(`[FLXTracker] Last poll: ${new Date(this.lastPollTime).toISOString()}`);
    }, 30000) as any;
  }
  ...
  private async updateTrackerStatus(status: string, errorMessage: string | null): Promise<void> {
    try {
      ...
      `).bind(
        ...
        now,
        errorMessage,
        0, // Felix hat keine Reconnects, immer 0
        now,
        'flx'
      ).run();
    } catch (error) {
      console.error('[FLXTracker] Failed to update tracker status:', error);
    }
  }
}
