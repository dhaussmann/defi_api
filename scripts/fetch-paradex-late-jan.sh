...
# Fetch Paradex funding rates for Jan 27 07:00 - Feb 2 20:00, 2026
...
TEMP_SQL="/tmp/paradex_late_jan_$(date +%s).sql"
> "$TEMP_SQL"
...
for MARKET in $MARKETS; do
  ...
  for TS in "${HOURLY_TIMESTAMPS[@]}"; do
    ...
    if [ "$HAS_RESULTS" -eq 1 ] 2>/dev/null; then
      ...
      # Paradex: 8-hour funding = 3 payments/day = * 3 * 365
      FUNDING_RATE=$(echo "$RESPONSE" | jq -r '.results[0].funding_rate | tonumber')
      FUNDING_RATE_ANNUAL=$(echo "$RESPONSE" | jq -r ".results[0].funding_rate | tonumber * 3 * 365")
    ...
    # Rate limiting: 1500 req/min = 25 req/sec
