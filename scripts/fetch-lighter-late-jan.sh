...
# Fetch Lighter funding rates for Jan 27 07:00 - Feb 2 20:00, 2026

START_S=1769493600  # Jan 27, 2026 07:00:00 UTC
END_S=1770000000    # Feb 2, 2026 20:00:00 UTC
COUNT_BACK=200      # Buffer for hourly data (~150 hours + buffer)
...
TEMP_SQL="/tmp/lighter_late_jan_$(date +%s).sql"
> "$TEMP_SQL"
...
while IFS=: read -r MARKET_ID SYMBOL; do
  ...
  if [ "$COUNT" -gt 0 ] 2>/dev/null; then
    ...
    # Lighter: 1-hour funding = 24 payments/day = * 24 * 365
    echo "$RESPONSE" | jq -r ".fundings[] | select(.timestamp >= $START_S and .timestamp <= $END_S) | \"INSERT OR REPLACE INTO market_history (exchange, symbol, normalized_symbol, avg_mark_price, avg_index_price, min_price, max_price, price_volatility, volume_base, volume_quote, avg_open_interest, avg_open_interest_usd, max_open_interest_usd, avg_funding_rate, avg_funding_rate_annual, min_funding_rate, max_funding_rate, hour_timestamp, sample_count, aggregated_at) VALUES ('lighter', '${SYMBOL}', '${SYMBOL}', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, \(.rate | tonumber / 100), \(.rate | tonumber / 100 * 24 * 365), \(.rate | tonumber / 100), \(.rate | tonumber / 100), \(.timestamp), 1, $(date +%s));\"" >> "$TEMP_SQL"
