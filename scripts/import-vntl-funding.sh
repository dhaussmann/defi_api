...
# VNTL (Trade.vntl) Historical Funding Rate Import
# ===============================================
# Imports funding rates from VNTL in 15-day chunks for the last 30 days.
#
# Usage:
#   ./import-vntl-funding.sh

set -e

API_URL="https://api.hyperliquid.vntl/info"
DB_NAME="defiapi-db"
REMOTE="--remote"
EXCHANGE="vntl"
DEX="vntl"

# Coins to import (only crypto assets have historical funding data on VNTL)
# Traditional finance instruments (stocks, commodities, forex) don't have historical data
COINS=("BTC" "ETH" "SOL")

# Time range: Start from December 1, 2025 (when VNTL DEX launched)
# VNTL is a new HIP-3 DEX that launched in December 2025
...
echo "=========================================="
echo "VNTL (Trade.vntl) Historical Funding Import"
echo "=========================================="
...
for COIN in "${COINS[@]}"; do
  ...
  while [ $CURRENT_START -lt $END_TIME ]; do
    ...
    echo "$RESPONSE" | jq -c '.[]' | while IFS= read -r record; do
      ...
      # Symbol formatting - API returns "vntl:COIN", use as-is
      SYMBOL="$COIN_NAME"
      # Normalized symbol without prefix
      NORMALIZED_SYMBOL=$(echo "$COIN_NAME" | sed 's/^vntl://')
