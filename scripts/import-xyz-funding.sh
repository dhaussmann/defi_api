...
# XYZ (Trade.xyz) Historical Funding Rate Import
# ===============================================
# Imports funding rates from XYZ in 15-day chunks for the last 30 days.
#
# Usage:
#   ./import-xyz-funding.sh
...
# Coins to import (only crypto assets have historical funding data on XYZ)
# Traditional finance instruments (stocks, commodities, forex) don't have historical data
COINS=("BTC" "ETH" "SOL")

# Time range: Start from January 4, 2026 (when XYZ DEX launched)
# XYZ is a new HIP-3 DEX that launched in early 2026
START_TIME=1736000000000  # Jan 4, 2026 in ms
CURRENT_TIME=$(date +%s)
...
echo "=========================================="
echo "XYZ (Trade.xyz) Historical Funding Import"
echo "=========================================="
...
for COIN in "${COINS[@]}"; do
  ...
  while [ $CURRENT_START -lt $END_TIME ]; do
    ...
    echo "$RESPONSE" | jq -c '.[]' | while IFS= read -r record; do
      ...
      # Symbol formatting - API returns "xyz:COIN", use as-is
      SYMBOL="$COIN_NAME"
      # Normalized symbol without prefix
      NORMALIZED_SYMBOL=$(echo "$COIN_NAME" | sed 's/^xyz://')
