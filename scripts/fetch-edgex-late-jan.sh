...
# Fetch EdgeX funding rates for Jan 27 07:00 - Feb 2 20:00, 2026
...
TEMP_SQL="/tmp/edgex_late_jan_$(date +%s).sql"
> "$TEMP_SQL"
...
while IFS='|' read -r CONTRACT_ID CONTRACT_NAME; do
  ...
  while true; do
    ...
    while IFS= read -r RECORD; do
      ...
      # EdgeX has 4-hour funding intervals
      # Annual rate = rate * (24/4) * 365 = rate * 6 * 365
      FUNDING_ANNUAL=$(echo "$FUNDING_RATE * 6 * 365" | bc -l)
