...
# Felix V3 Historical Funding Rate Import
# ========================================
# Fetches historical funding rates from Felix (Hyperliquid API with dex: flx).
# Based on Paradex V3 import script pattern.
#
# Strategy:
# - Fetch active markets via POST {type: "metaAndAssetCtxs", dex: "flx"}
# - Use POST {type: "fundingHistory", coin: "flx:XXX"} for historical data
# - Paginate with startTime/endTime (max 500 per page)
# - Import into felix_funding_v3 table
#
# Usage:
#   ./import-felix-v3.sh [DAYS_BACK]
...
# V3 Config: Felix uses 1-hour intervals (same as Hyperliquid)
INTERVAL_HOURS=1
...
# Hyperliquid API uses milliseconds
...
echo "=========================================="
echo "Felix V3 Historical Funding Rate Import"
echo "=========================================="
echo "Date range: $START_DATE to $END_DATE ($DAYS_BACK days)"
echo "Target table: felix_funding_v3"
echo "API: $API_URL (dex: flx)"
echo "Interval: ${INTERVAL_HOURS}h"
echo ""

# Get all active Felix markets
echo "[1/3] Fetching Felix markets..."
MARKETS=$(curl -s "$API_URL" \
  -X POST \
  -H "Content-Type: application/json" \
  -d '{"type":"metaAndAssetCtxs", "dex": "flx"}' | \
...
for COIN in $MARKETS; do
  NUM=$((NUM + 1))
  BASE_ASSET=$(echo "$COIN" | sed 's/^flx://')
  ...
  while true; do
    ...
    # Generate SQL
    # .time = funding time in ms, .fundingRate = decimal rate (1h)
    echo "$RESPONSE" | jq -r --arg sym "$COIN" --arg base "$BASE_ASSET" \
      ...
      "INSERT OR REPLACE INTO felix_funding_v3 (symbol, base_asset, funding_time, rate_raw, rate_raw_percent, interval_hours, rate_1h_percent, rate_apr, collected_at, source) VALUES (\"\($sym)\", \"\($base)\", \($ts), \($rate_raw), \($rate_pct), \($interval), \($rate_1h), \($rate_apr), \($collected), \"import\");"' >> "$OUTPUT_FILE"
