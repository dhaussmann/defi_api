...
# KM (Trade.km) Historical Funding Rate Import
# ===============================================
# Imports funding rates from KM in 15-day chunks for the last 30 days.
#
# Usage:
#   ./import-km-funding.sh

set -e

API_URL="https://api.hyperliquid.km/info"
DB_NAME="defiapi-db"
REMOTE="--remote"
EXCHANGE="km"
DEX="km"

# Coins to import (only crypto assets have historical funding data on KM)
# Traditional finance instruments (stocks, commodities, forex) don't have historical data
COINS=("BTC" "ETH" "SOL")

# Time range: Start from December 1, 2025 (when KM DEX launched)
# KM is a new HIP-3 DEX that launched in December 2025
...
echo "=========================================="
echo "KM (Trade.km) Historical Funding Import"
echo "=========================================="
...
for COIN in "${COINS[@]}"; do
  ...
  while [ $CURRENT_START -lt $END_TIME ]; do
    ...
    echo "$RESPONSE" | jq -c '.[]' | while IFS= read -r record; do
      ...
      # Symbol formatting - API returns "km:COIN", use as-is
      SYMBOL="$COIN_NAME"
      # Normalized symbol without prefix
      NORMALIZED_SYMBOL=$(echo "$COIN_NAME" | sed 's/^km://')
